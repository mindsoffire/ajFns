    ...
    if (tokenHdr !== undefined) {
        ...
        if (token[1]) {
            ...
            try {
                ...
                let testSecret = aj.f_rotajG(subParts[0]) + '==' + aj.sha256(subParts[1]);
                ...
                try {
                    ...
                    res.nexToken = aj.f_genIssueToken(subParts[0], subParts[1], verified.acl);
...
/* function f_genIssueToken(encID, hshPW, perms) {
    let iat = Date.now();
    let jwtPayload = {
        sub: encID + '##' + hshPW,
        iat: iat,
    ...
    let secret = '';
    encID ? secret = aj.f_rotajG(encID) : secret;
...
console.log(f_genIssueToken('jskJIrKKswx@LjHsP.nrj', '3cc98b78aabdbb22273c4679870ad2e05c8672707675734f0ea2fbef6618d651', 'rw_'));
console.log(f_genIssueToken('jskJIrKKswx@LjHsP.nrj', null, 'rw_'));
console.log(f_genIssueToken('jskJIrKKswx@LjHsP.nrj', '', 'rw_')); */
